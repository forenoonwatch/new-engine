...
void main() {
	...
	//lightDir = TBN * normalize(cameraPosition - localPos);

	gl_Position = vertPos;
	texCoord0 = texCoord * 50.0;
	clipSpace = vertPos;
	fresnel = clamp(dot(N, normalize(cameraPosition - vertPos.xyz)), 0.0, 1.0);
}

#elif defined(FS_BUILD)

#define WAVE_STRENGTH 0.03
...
void main() {
	...
	//normal = normalize(mad(normal, 2.0, -1.0));
	//normal = normalize(normal);
	const vec3 normal = vec3(0, 0, 1);

	vec2 distort = mad(texture2D(dudvMap, texCoord0).rg, 2.0, -1.0) * WAVE_STRENGTH;
	distort.x *= 0.25 * sin(WAVE_SPEED * time);
	...
	vec3 flect = texture2D(reflection, vec2(ndc.x, -ndc.y) + distort).rgb;
	vec3 fract = texture2D(refraction, ndc + distort).rgb;

	//color *= mix(flect, fract, fresnel);
	//color = mix(color, vec3(0.372549, 0.61176, 0.60784), 0.7);
	color *= mix(mix(flect, fract, fresnel), vec3(0, 0.15, 0.3), 0.7);
