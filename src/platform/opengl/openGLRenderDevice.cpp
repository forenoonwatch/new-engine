...
static void GLAPIENTRY GLErrorCallback(GLenum source,
	...
	if (type == GL_DEBUG_TYPE_ERROR) {
		DEBUG_LOG_TEMP("GL ERROR: type = 0x%x, severity = 0x%x, message = %s\n",
...
uint32 OpenGLRenderDevice::releaseRenderTarget(uint32 fbo) {
	...
	HashMap<uint32, FBOData>::iterator it = fboMap.find(fbo);
	if (it == fboMap.end()) {
		return 0;
	}
...
uint32 OpenGLRenderDevice::createGeometryBuffer(int32 width, int32 height) {
	struct GBuffer buffer;

	glGenFramebuffers(1, &buffer.fbo);
	setFBO(buffer.fbo);

	glGenTextures(GBUFFER_NUM_TEXTURES, buffer.textures);
	...
	if (status != GL_FRAMEBUFFER_COMPLETE) {
		DEBUG_LOG(LOG_ERROR, LOG_TYPE_RENDERER, "GBuffer Framebuffer status: 0x%x", status);
		return 0;
	}
#endif

	geometryBufferMap[buffer.fbo] = buffer;

	return buffer.fbo;
}

uint32 OpenGLRenderDevice::releaseGeometryBuffer(uint32 fbo) {
	if (fbo == 0) {
		return 0;
	}

	HashMap<uint32, GBuffer>::iterator it = geometryBufferMap.find(fbo);

	if (it == std::end(geometryBufferMap)) {
		return 0;
	}

	glDeleteFramebuffers(1, &fbo);

	if (it->second.textures[0] != 0) {
		glDeleteTextures(ARRAY_SIZE_IN_ELEMENTS(it->second.textures), it->second.textures);
	}

	if (it->second.depthTexture != 0) {
		glDeleteTextures(1, &it->second.depthTexture);
	}
	
	geometryBufferMap.erase(it);
...
uint32 OpenGLRenderDevice::createSampler(enum SamplerFilter minFilter, enum SamplerFilter magFilter,
	...
	glSamplerParameteri(result, GL_TEXTURE_WRAP_R, wrapV);
...
uint32 OpenGLRenderDevice::createDDSTexture2D(uint32 width, uint32 height,
		const unsigned char* buffer, uint32 fourCC, uint32 mipMapCount) {
	GLint format;
...
uint32 OpenGLRenderDevice::createCubeMap() {
	uint32 textureID;
	glGenTextures(1, &textureID);
...
void OpenGLRenderDevice::addTextureToCubeMap(uint32 cubeMap, uint32 face,
		int32 width, int32 height, const void* data) {
	glTexImage2D(face, 0, GL_RGB, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);
}

void OpenGLRenderDevice::setTexturePackAlignment(uint32 packAlignment) {
...
void OpenGLRenderDevice::updateUniformBuffer(uint32 buffer, const void* data,
		uintptr offset, uintptr dataSize) {
	glBindBuffer(GL_UNIFORM_BUFFER, buffer);

	void* dest = glMapBuffer(GL_UNIFORM_BUFFER, GL_WRITE_ONLY);
	Memory::memcpy(reinterpret_cast<void*>(reinterpret_cast<uintptr>(dest) + offset), data, dataSize);
	glUnmapBuffer(GL_UNIFORM_BUFFER);
}
...
uint32 OpenGLRenderDevice::releaseShaderProgram(uint32 shader) {
	...
	HashMap<uint32, ShaderProgram>::iterator programIt = shaderProgramMap.find(shader);
...
static void addShaderUniforms(GLuint shaderProgram, const String& shaderText,
	...
	for (int32 uniform = 0; uniform < numUniforms; ++uniform) {
		...
		if (type != GL_SAMPLER_2D && type != GL_SAMPLER_CUBE) {
			//DEBUG_LOG(LOG_TYPE_RENDERER, LOG_ERROR,
			//		"Non-sampler uniforms currently unsupported!: %s", (char*)&uniformName[0]);
			continue;
		}
