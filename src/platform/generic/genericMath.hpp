...
//	}
	...
	static FORCEINLINE T bilerp(const T& val00, const T& val10,
		...
		return lerp(lerp(val00, val10, amtX),
				lerp(val01, val11, amtX), amtY);
	}
	...
	template <typename T>
	static FORCEINLINE T mod(const T& val1, const T& val2) {
		return val1 % val2;
	}

	template <typename T>
	static FORCEINLINE T floor(const T& val) {
		return ::floor(val);
	}

	template <typename T>
	static FORCEINLINE T fract(const T& val) {
